	...
	<tr>
		<td class="pad">
			<div style="color:#000000;direction:ltr;font-family:Arial, 'Helvetica Neue', Helvetica, sans-serif;font-size:15px;font-weight:400;letter-spacing:0px;line-height:120%;text-align:left;mso-line-height-alt:18px;">
				<h5>Hey {{ $order->user->full_name }} </h5>
				<p style="margin: 0;">Thank you for purchasing game serial keys on our platform!.</p>
...
<table class="button_block block-3" width="100%" border="0" cellpadding="10" cellspacing="0" role="presentation" style="mso-table-lspace: 0pt; mso-table-rspace: 0pt;">
	...
	@foreach($order->keys as $game_key):
		<tr>
			<th>{{ $game_key->name }}</th>
			<th>{{ $game_key->serial }}</th>
		</tr>
	@endforeach
</table>
	...
	<tr>
		<td class="pad">
			<div class="alignment" align="center">
				...
				<a href="{{ url('/login') }}" style="text-decoration:none;display:inline-block;color:#ffffff;background-color:#3c9986;border-radius:4px;width:auto;border-top:1px solid #3c9986;font-weight:400;border-right:1px solid #3c9986;border-bottom:1px solid #3c9986;border-left:1px solid #3c9986;padding-top:5px;padding-bottom:5px;font-family:Arial, Helvetica Neue, Helvetica, sans-serif;text-align:center;mso-border-alt:none;word-break:keep-all;"><span style="padding-left:20px;padding-right:20px;font-size:14px;display:inline-block;letter-spacing:normal;"><span dir="ltr" style="word-break: break-word; line-height: 28px;"><strong>Login</strong></span></span></a>
